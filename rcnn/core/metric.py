...
def get_rpn_bbox_names():
    pred = ['rpn_cls_prob', 'rpn_bbox_loss', 'rpn_label', 'rpn_bbox_weight']
    label = ['rpn_label', 'rpn_bbox_target', 'rpn_bbox_weight']
    return pred, label

def get_rpn_kpoint_names():
    pred = ['rpn_cls_prob', 'rpn_kpoint_loss', 'rpn_label', 'rpn_kpoint_weight']
    label = ['rpn_label', 'rpn_kpoint_target', 'rpn_kpoint_weight']
    return pred, label
...
def get_rcnn_names():
    ...
    if config.TRAIN.END2END:
        ...
        rpn_pred, rpn_label = get_rpn_bbox_names()
        pred = rpn_pred + pred
        label = rpn_label
    return pred, label
...
class RPNAccMetric(mx.metric.EvalMetric):
    def __init__(self, pred_idx=-1, label_idx=-1,):
        super(RPNAccMetric, self).__init__('M{}_RPNAcc'.format(pred_idx//6))
        self.pred, self.label = get_rpn_bbox_names()
        self.stride = pred_idx//6
        self.name = ['M{}_'.format(self.stride)+'RPNAcc', 'M{}_'.format(self.stride)+'RPNAccBG', 'M{}_'.format(self.stride)+'RPNAccFG']
    ...
    def update(self, labels, preds):
        '''
        通过使用softmax之后得到的正负样本的概率作为预测值，然后和label值进行比较，分别得到总的，正样本的，负样本的三个acc
        :param labels:
        :param preds:
        :return:
        '''
        ...
        # pred (b, c, p) or (b, c, h, w) 将softmax得到的0和１的概率进行合并，哪个概率高就是哪一类
...
class RPNL1LossMetric(mx.metric.EvalMetric):
    def __init__(self, loss_idx=-1, weight_idx=-1):
        super(RPNL1LossMetric, self).__init__('M{}_RPNL1BboxLoss'.format(loss_idx//6))
        self.pred, self.label = get_rpn_bbox_names()
    ...
    def update(self, labels, preds):
        '''
        每个stride下都会对应一个metric
        labels:在dataIter中生成的每个batchsize的labels,其中包裹每个stride下的label,bbox_target,bbox_weight,kpoint_target,kpoint_weight
        preds:
        label = ['rpn_label', 'rpn_bbox_target', 'rpn_bbox_weight']
        pred = ['rpn_cls_prob', 'rpn_bbox_loss', 'rpn_label', 'rpn_bbox_weight']
        '''
        ...
        num_inst = np.sum(bbox_weight > 0) / 4      #inside并且为正样本
...
class RPNL1LossMetricKpoint(mx.metric.EvalMetric):
    def __init__(self, loss_idx=-1, weight_idx=-1):
        super(RPNL1LossMetricKpoint, self).__init__('M{}_RPNL1KpointLoss'.format(loss_idx//6))
        self.pred, self.label = get_rpn_kpoint_names()
        self.loss_idx = loss_idx
        self.weight_idx = weight_idx
